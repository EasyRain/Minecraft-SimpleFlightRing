...
public final class RaidRingQuest {
    ...
    public static void onGolemDeath(LivingDeathEvent event) {
        ...
        if (!ring.isEmpty()) {
            ring.set(ModDataComponents.ILLAGER_ACKNOWLEDGED.get(), Unit.INSTANCE);
        }
    ...
    private static void showOverlay(ServerPlayer player, String key, Object... args) {
        player.sendOverlayMessage(Component.translatable(key, args).withColor(RingAbility.RAID_PLUNDER.color()));
    }
