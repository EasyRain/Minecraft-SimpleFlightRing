...
损坏版每个符合条件的箱子 **1%** 概率开出（绿宝石戒指不走战利品，改由大师级图书管理员出售，
且**每只管理员只有 50% 概率**刷出该交易）。

| 戒指 | 获取途径（每箱 1%） | 修复材料 | 飞行时间 | 耐久 |
|------|--------------------|---------|---------|------|
| 幽匿飞行戒指 | 远古城市宝箱 | 见下方任务链 | 120 分钟 | 7200 |
| 矿工飞行戒指 | 废弃矿井箱子 / 箱子矿车 | 见下方任务链 | 30 分钟 | 1800（自带耐久 I） |
| 绿宝石飞行戒指 | **大师级图书管理员**（32 绿宝石，3 次，50% 概率） | 绿宝石块 | 60 分钟 | 3600 |
...
- 修复配方是**工作台无序合成**：损坏戒指 + 1 个对应主题材料 → 完好的戒指，之后与普通戒指完全一致
  （可附魔、可放 Curios 槽、可再用「不毁核心」变成不毁）。
- 1% 是**每次开箱独立判定**，在原本的战利品之外额外掉落。
...
绿宝石戒指没有任务链：从大师级图书管理员手里买到损坏版，用绿宝石块修好即可
（**每只管理员只有 50% 概率**刷出这笔交易，晋升大师时判定一次，没出就换一位）。
它的能力是**纯被动**的，戴上戒指（Curios 飞行戒指槽）即生效，耐久耗尽或摘下立刻失效：
