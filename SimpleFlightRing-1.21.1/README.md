...
| 矿工飞行戒指 | 废弃矿井箱子 / 箱子矿车 | 铁块 | 90 分钟 | 5400 |
...
1. 刚拿到时 tooltip 为灰色**「戒指已失去了远古的力量」**+ 幽匿蓝**「戒指在渴望坚守者的灵魂」**；
2. **携带这枚戒指击杀坚守者**（背包 / 副手 / Curios 槽都算）——一只坚守者只对应一枚戒指，
   多枚损坏戒指需要分别去杀各自的坚守者。吞噬灵魂后戒指**带附魔光效**，第二行变为幽匿蓝
   **「戒指需要一个新的躯壳」**；
3. 在工作台用**围一圈 8 个回响碎片 + 中间那枚已充能的戒指**合成完整的幽匿飞行戒指。

> 只有"已充能（吞噬过灵魂）"的损坏戒指才能参与该配方，未充能的放上去不出货；配方在 JEI 里可查。
> 创造模式物品栏里也放了一枚已充能的损坏戒指，方便直接试配方（不需要时可以从创造栏条目里删掉）。

## 戒指能力键（默认 V）

- 新增按键绑定「触发戒指能力」，默认 **V**，位于「按键设置 → 杂项」，可自由改键。
- 该键用于触发特殊戒指的**主动**能力；目前实现的能力（魔法内衬 / 动能偏转 / 爆裂图腾）都是自动触发的被动效果，
  因此按键目前只会提示「XX 暂无主动能力」，属于占位用途。
