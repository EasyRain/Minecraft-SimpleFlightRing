...
public final class BackpackCompat {
    ...
    public record BackpackRing(IBackpackWrapper wrapper, int slotIndex) {
        public ItemStack stack() {
            return wrapper.getInventoryHandler().getStackInSlot(slotIndex);
        }
    }
    ...
    public static List<BackpackRing> findRingsInBackpacks(Player player) {
        ...
        scanBackpack(player.getInventory().offhand.get(0), rings, 0);
    ...
    public static void writeBack(BackpackRing ring) {
        // setStackInSlot triggers the backpack's contents-change handler, which
        // serializes the inventory back into the backpack item's data component.
        ring.wrapper().getInventoryHandler().setStackInSlot(ring.slotIndex(), ring.stack());
    }
    ...
    private static void scanBackpack(ItemStack stack, List<BackpackRing> rings, int depth) {
        ...
        IItemHandlerModifiable handler = wrapper.getInventoryHandler();
